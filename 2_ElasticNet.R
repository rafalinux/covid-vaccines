...
#########################################
#                                       #
# TIME SERIES con MACHINE LEARNING (I)  #
#                                       #
#########################################
# Reference URL_
...
#         engines only). For keras models, this corresponds to purely L2 regularization
#         (aka weight decay) while the other models can be either or a combination of L1
#         and L2 (depending on the value of mixture).
...
# 4. MAKING PREDICTIONS (with training set)

# Do predictions fit well with observed data?
...
ggplot(aes(x = date), data = total_tbl) +
  ...
  labs(title = titulo,
       subtitle="Fitted predictions using ElasticNet penalized regression")+  
  theme(plot.title = element_text(hjust = 0.5)) 

# 6. MAKING PREDICTIONS/FORECASTING (with testing set)
...
ggplot(aes(x = date), data = total_tbl) +
  ...
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank() ) +
  labs(title = titulo,
       subtitle="Forecast using ElasticNet penalized regression")+  
  theme(plot.title = element_text(hjust = 0.5)) 

# 8. FORECASTING TWO LINES
...
# 9. ADDING MOVING AVERAGE AND CONFIDENCE INTERVAL
...
ggplot(data = predicciones_ingresos, aes(x=date) ) +
  ...
  labs(title = "Hospitalizations averted by COVID-19 vaccinations with the ElasticNet model",
